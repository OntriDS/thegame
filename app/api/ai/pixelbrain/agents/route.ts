...
/**
 * Proxy Pixelbrain agent catalog for Research AI Assistant dropdown (admin-only).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = `${base}/api/orchestration/agents`;
    ...
    if (!res.ok) {
      return Response.json(
        { error: 'Failed to fetch Pixelbrain agents', pixelbrainStatus: res.status, body: text.slice(0, 2000) },
    ...
    console.error('[GET /api/ai/pixelbrain/agents]', e);
