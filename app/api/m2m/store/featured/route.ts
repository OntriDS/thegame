...
function toStoreItemPayload(item: Item) {
  return {
    ...
    collection: item.context?.collection,
    dimensions: item.context?.dimensions,
    size: item.context?.size,
    status: item.status,
    price: item.pricing?.targetPrice,
    year: item.context?.year,
    media: {
      main: cdnUrlForObjectKey(item.media?.mainUrl),
      thumb: cdnUrlForObjectKey(item.media?.thumbUrl),
      gallery: item.media?.galleryUrls?.map((url) => cdnUrlForObjectKey(url)) || [],
    },
