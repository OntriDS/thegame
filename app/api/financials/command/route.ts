// app/api/financials/command/route.ts
// Increment 7: Command-based FinancialRecord API
//
...
//
// The legacy POST /api/financials route remains active during migration.
// New code should use this command route.

import { NextRequest, NextResponse } from 'next/server';
import { requireAdminAuth } from '@/lib/api-auth';
import { executeFinancialRecordCommand } from '@/lib/domain/commands/financial-record-command-handler';
import type { EntityCommandEnvelope, FinancialRecordCommand, CommandOutcomeV1 } from '@/lib/domain/commands/contracts';
import { v4 as uuid } from 'uuid';

export const dynamic = 'force-dynamic';

/**
 * POST /api/financials/command
 *
 ...
 *   payload: FinancialRecordCommand  // Required: the command to execute
 * }
 *
 * Response:
 * {
 *   commandId: string,
 *   aggregate: { type: 'financial', id: string },
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const envelope: EntityCommandEnvelope<FinancialRecordCommand> = {
    ...
    // Execute command
    const outcome: CommandOutcomeV1 = await executeFinancialRecordCommand(envelope);
    ...
    if (outcome.state === 'failed') {
      ...
      } else if (outcome.errorCode === 'FINREC_NOT_FOUND') {
        httpStatus = 404;
      } else {
        httpStatus = 400;
      }
    ...
    console.error('[API] Error executing financial record command:', error);
    return NextResponse.json(
