...
describe('entity-test: minimal Booth Sale', () => {
  const saleId = 'entity-test-sale-booth-clean';
  ...
  it('persists the minimum Booth Sale shape without partner, contract, payments, or rewards', async () => {
    const now = getUTCNow();
    const items = await getAllItems();
    const sites = await getAllSites();
    const item = items.find(candidate => candidate.status !== 'sold');
    const site = sites.find(candidate => candidate.id === 'hq') || sites[0];
    if (!item || !site) throw new Error('Cannot run Booth Sale test: an Item and Site are required.');

    await upsertSale({
      id: saleId,
      schemaVersion: 1,
      version: 0,
      name: 'testing-booth-sale',
      type: SaleType.BOOTH,
    ...
    writeFileSync(
      resolve(__dirname, 'booth-sale-entity-test-clean.output.json'),
      `${JSON.stringify(persisted, null, 2)}\n`,
      'utf8',
    );
    process.stderr.write(`[entity-test] persisted minimal Booth Sale:\n${JSON.stringify(persisted, null, 2)}\n`);
    ...
    expect(links).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        ...
        target: { type: EntityType.ITEM, id: item.id },
      }),
    ]));
