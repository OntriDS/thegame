...
export default function SalesModal({
  ...
  // Auto-assign Site Owner as Customer when the selected Site changes
  useEffect(() => {
    if (!open) return;
    if (type !== SaleType.NETWORK && type !== SaleType.DIRECT) return;
    
    const currentSiteId = normalizeSiteId(siteId);
    
    // Ignore if this is the initial load for an existing sale
    if (saleSiteHydrationKeyRef.current === currentSiteId) return;
    if (!currentSiteId || currentSiteId === 'none') return;
    
    let cancelled = false;
    ClientAPI.getLinksFor({ type: EntityType.SITE, id: currentSiteId }).then(links => {
      if (cancelled) return;
      
      // Look for a CHARACTER_SITE link where the site is the target
      const ownerLink = links.find(l => 
        l.linkType === LinkType.CHARACTER_SITE && 
        l.target.type === EntityType.SITE && 
        l.target.id === currentSiteId
      );
      
      if (ownerLink && ownerLink.source.type === EntityType.CHARACTER) {
        const ownerCharacterId = ownerLink.source.id;
        // Check current characterId via callback to avoid dependency loop
        setCharacterId(prev => {
          if (prev !== ownerCharacterId) {
            setIsNewCustomer(false);
            setNewCustomerName('');
  ...
  // Auto-assign Customer when Site changes
  useEffect(() => {
    if (!open) return;
    if (type !== SaleType.NETWORK && type !== SaleType.DIRECT) return;
  ...
  // Auto-assign Site when Customer changes
  useEffect(() => {
    if (!open) return;
    if (type !== SaleType.NETWORK && type !== SaleType.DIRECT) return;
    ...
    let cancelled = false;
    ClientAPI.getLinksFor({ type: EntityType.CHARACTER, id: characterId }).then(links => {
      if (cancelled) return;
      ...
      // Only auto-assign if they own exactly ONE site
      if (siteLinks.length === 1 && siteLinks[0].target.type === EntityType.SITE) {
        const ownedSiteId = siteLinks[0].target.id;
        setSiteId(prev => {
          if (normalizeSiteId(prev) !== normalizeSiteId(ownedSiteId)) {
            return ownedSiteId;
          }
          return prev;
        });
  ...
  const handleSave = async (overrideSale?: Sale, force: boolean = false) => {
    ...
    // Product acquisition/production cost is already recorded when the item
