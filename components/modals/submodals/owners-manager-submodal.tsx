...
interface OwnersManagerSubmodalProps {
  open: boolean;
...
export default function OwnersManagerSubmodal({
  open,
  onOpenChange,
  entityType,
  entityId,
  ...
  linkType,
  onOwnersChanged
}: OwnersManagerSubmodalProps) {
  const [characters, setCharacters] = useState<Character[]>([]);
  const [currentOwners, setCurrentOwners] = useState<Character[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  // Load characters and current owners when modal opens
  useEffect(() => {
    if (open) {
      loadData();
    }
  ...
  const loadData = async () => {
    try {
      ...
      // Load current owners from links
      ...
      // Fetch character details for owners
      const owners = activeCharacters.filter((c: Character) => ownerIds.has(c.id));
  ...
  // Filter characters based on search term (exclude current owners)
  const currentOwnerIds = new Set(currentOwners.map(c => c.id));
  const filteredCharacters = characters.filter((c: Character) => {
    const matchesSearch = c.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      c.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      c.contactEmail?.toLowerCase().includes(searchTerm.toLowerCase());
    const isNotOwner = !currentOwnerIds.has(c.id);
    return matchesSearch && isNotOwner;
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Current Owners */}
          <div>
            <h3 className="text-sm font-semibold mb-2">Current Owners ({currentOwners.length})</h3>
